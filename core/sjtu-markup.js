...
function parseSJTUMarkup(source) {
  ...
  while (index < lines.length) {
    ...
    if (blockMatch) {
      const startLine = currentLine();
      const titleInfo = parseEditorIdSuffix(blockMatch[2] || blockMatch[1]);
      const body = [];
      index += 1;
      while (index < lines.length && lines[index].trim() !== ":::") {
        body.push(lines[index]);
      ...
      index += 1;
      pushFragment(markFragment(
        makeMarkupBlock(blockMatch[1], titleInfo.text || blockMatch[1], body.join("\n"), footnoteDefinitions),
        startLine,
        previousLine(),
        blockMatch[1],
        titleInfo.text || blockMatch[1],
        titleInfo.id
      ));
      continue;
    }
...
function parseMarkupContentGroupHeaderWithId(line) {
  const groups = Array.from(line.matchAll(/\[([^\]]*)\]/g), (match) => match[1].trim());
  const parsed = parseEditorIdSuffix(line.replace(/^###\s*/, "").replace(/\s*\[[^\]]*\]/g, "").trim());
  const effects = new Set(["fade", "fade-up", "rise", "slide-left", "slide-right", "zoom", "blur", "move-between"]);
  const options = {};
  for (const group of groups) {
    if (effects.has(group)) options.effect = group;
    else if (/^step\s*=/.test(group)) options.appear = Number(group.split("=")[1]);
